...
- Always show players readable Discord display text: prefer the server nickname, then global display name, then the account name when no display name exists. Use `resolve_guild_player_names` for live rosters; it fetches members missing from the gateway cache. For synchronous paths use `cached_guild_player_names`. Both fall back to the last display name the bot saw (`player_display_names`, mirrored by `PlayerNameArchive`), which never implies guild membership. Never fall back to a numeric Discord ID, mention markup, or an unvalidated stored/DB name such as the registration name. If no live or last-seen name exists, show `Unknown player`. Apply `readable_player_name` at rendering boundaries to reject historical ID/mention fallbacks. This has regressed more than once — check cold caches, lookup failures, and missing player records explicitly.
- To silently subscribe a user to a thread (so they see new activity without opening it manually), post a message that `@mentions` them with `DiscordAllowedMentions::None` (`DiscordMessage::silent`, or an equivalent explicit user allowlist) rather than calling the thread-member API (`add_thread_member`) directly. Discord treats a mention in message content as an organic join and adds no extra text; the explicit thread-member API always posts its own unsuppressable "X added Y to the thread" system message on top of anything the bot sends, which reads as a confusing duplicate. `allowed_mentions: None` only silences the ping that the mention(s) in that message would otherwise trigger — it is per-message and per-mention, not a broader notification switch: the mention is still client-rendered and can appear unresolved, so it must not substitute for readable roster or status text; and it has no effect on any other message's mentions or on non-mention notification sources (e.g. a channel's general unread/activity indicators).
