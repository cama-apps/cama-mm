...
- Always show players readable Discord display text: prefer the server nickname, then global display name, then the account name when no display name exists. Use `resolve_guild_player_names` for live rosters; it fetches members missing from the gateway cache. Never fall back to a numeric Discord ID, mention markup, or an unvalidated stored/DB name. If Discord cannot provide a usable name, show `Unknown player`. Apply `readable_player_name` at rendering boundaries to reject historical ID/mention fallbacks. This has regressed more than once — check cold caches, lookup failures, and missing player records explicitly.
