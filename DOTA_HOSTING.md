...
When the GC omits the game-rules phase, validated Valve live data can also
confirm gameplay: a playable phase, or a positive game clock when the live
phase is absent. The sample must match the registered match and expected
accounts. The host persists betting closure before snapshots, spectator
commentary, or map capture become available; collecting this start evidence
does not expose live details while wagers remain open. Polling health logs
identify the source and a fixed status label, with changes logged immediately
and unchanged statuses repeated at most every five minutes.
...
`0 eligible viewers` means no private spectator space is needed; a nonplaying
viewer must opt in with 📻. It does not disable recap capture. The worker still
captures qualified maps after betting closes, even when nobody has subscribed.
...
After the recorded match's enriched summary is successfully posted, an optional **separate map recap GIF** follows in that same channel. It uses qualified live map screenshots captured by the spectator worker every 15 advancing game seconds, even with no subscribers or unavailable Discord delivery. Capture requires closed betting, a valid bot-hosted roster, and a fresh matching map frame; it never posts live information publicly. Encoding selects up to 88 distinct chronological samples, preserving the first and last, with half-second holds and a one-second final hold: at most **44.5 seconds**, shorter for smaller captures. A complete 30–60 minute capture therefore plays at approximately 40–80×. Discrete map observations are never interpolated. The message states the captured game-clock range so partial coverage is visible.
...
The recap outbox is queued only after a successful summary receipt with the matching Valve ID. Publication also requires the canonical recorded match to match its guild/pending/Valve identity and pending finalization to be complete; these checks repeat after encoding. Lost upload responses recover by durable delivery nonce and bounded history lookup. An inconclusive history lookup postpones delivery rather than duplicating it. Recap failures never block result recording, betting settlement, or summary publication. Manual matches, games without saved screenshots, disabled/unavailable enrichment, and failed summary publication produce no recap. The existing summary task is not itself a durable outbox: a process exit between summary acknowledgment and recap enqueue can omit this optional recap. Once enqueued, retries survive restart while the spectator worker is enabled. Server INFO logs report eligible viewer count, feed readiness/staleness, capture and private-delivery progress, and recap queue/delivery or missing-frame reasons. Repeated spectator health messages are limited to once every five minutes per match/component, with immediate state changes. Completed hosting status updates rejected because a thread is archived are retired once; active/review status updates keep retrying.
...
A realtime scoreboard without map positions still triggers the league-map fallback. A qualified map is preferred; if the league feed has no map or fails, usable realtime commentary is retained. Sources are selected as complete snapshots without mixing their game clocks or positions.
...
The spectator channel also has one map embed, edited in place from qualified advancing map samples independently of commentary updates. A newer scoreboard does not invalidate a recently captured map while Discord delivery is in progress; a queued frame expires after 90 seconds, and the image retains its original game timestamp. Hero world positions and respawn timers come from one qualified Valve sample, without interpolating or combining positions from different samples. Missing/invalid coordinates are omitted, all-zero placeholder rosters are rejected, and stale/cached clocks do not advance the map. The image carries its game timestamp. Rendering runs off Tokio's worker threads; map publication independently rechecks spectator access, current roster, closed betting, and feed freshness. Map failures do not block text updates. Message identity is retained for recovery and replacement PNG attachments do not accumulate.
