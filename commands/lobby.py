...
class ReadyCheckView(discord.ui.View):
    """Persistent view with ready button for ready checks."""
    ...
    async def ready_button(
        ...
        # Update embed (will be handled by match commands cog)
        # Trigger embed update by getting match commands cog
        ...
        # Check if all ready - trigger shuffle continuation
        if ready_check.is_complete():
            logger.info(f"All players ready for guild {self.guild_id}")
            if match_cog and hasattr(match_cog, "_on_ready_check_complete"):
                try:
                    thread_id = self.bot.lobby_service.get_lobby_thread_id()
                    if thread_id:
                        thread = await self.bot.fetch_channel(thread_id)
                        await match_cog._on_ready_check_complete(thread, self.guild_id)
                except Exception as exc:
