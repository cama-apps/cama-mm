...
class PredictionCommands(commands.Cog):
    ...
    @app_commands.command(name="predictionstats", description="View prediction betting stats")
    @app_commands.describe(user="User to view stats for (default: yourself)")
    async def predictionstats(
        self,
        interaction: discord.Interaction,
        user: discord.Member | None = None,
    ):
        """View prediction betting stats for a user."""
        await safe_defer(interaction)

        target = user or interaction.user
        stats = self.prediction_service.prediction_repo.get_user_prediction_stats(target.id)

        if not stats:
            await safe_followup(
                interaction,
                content=f"{target.display_name} has no prediction betting history yet.",
            )
            return

        # Build embed
        pnl = stats["net_pnl"] or 0
        pnl_color = 0x57F287 if pnl >= 0 else 0xED4245  # Green or red

        embed = discord.Embed(
            title=f"🔮 Prediction Stats for {target.display_name}",
            color=pnl_color,
        )
    ...
    @app_commands.command(name="predictionleaderboard", description="View prediction betting leaderboard")
    @app_commands.describe(limit="Number of entries to show (default 5)")
    async def predictionleaderboard(
        self,
        interaction: discord.Interaction,
        limit: int = 5,
    ):
        """View the prediction betting leaderboard."""
        await safe_defer(interaction)
