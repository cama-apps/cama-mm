...
@pytest.fixture
def garnishment_service(player_repo):
    """Create a GarnishmentService with 100% garnishment rate."""
    return GarnishmentService(player_repo, garnishment_rate=1.0)


@pytest.fixture
def garnishment_service_50_percent(player_repo):
    """Create a GarnishmentService with 50% garnishment rate."""
    return GarnishmentService(player_repo, garnishment_rate=0.5)


@pytest.fixture
def test_player(player_repo):
...
class TestGarnishmentService:
    ...
    def test_no_garnishment_when_positive_balance(
        self, garnishment_service, player_repo, test_player
    ):
        ...
        balance = player_repo.get_balance(test_player, TEST_GUILD_ID)
        assert balance == 103
    ...
    def test_full_garnishment_when_in_debt(
        ...
        # Put player in debt
        player_repo.update_balance(test_player, TEST_GUILD_ID, -50)
    ...
    def test_partial_garnishment_with_50_percent_rate(
        self, garnishment_service_50_percent, player_repo, test_player
    ):
        """Test that partial garnishment is applied with 50% rate."""
        # Put player in debt
        player_repo.update_balance(test_player, TEST_GUILD_ID, -100)
...
class TestGarnishmentTransitions:
    ...
    def test_income_pays_off_debt_completely(
        ...
        player_repo.update_balance(test_player, TEST_GUILD_ID, -30)
    ...
    def test_exactly_at_zero_balance(
        ...
        player_repo.update_balance(test_player, TEST_GUILD_ID, 0)
...
class TestGarnishmentRates:
    ...
    def test_default_garnishment_rate_uses_config(self, player_repo):
        """Test that default garnishment rate uses config value."""
        from config import GARNISHMENT_PERCENTAGE

        service = GarnishmentService(player_repo)
        assert service.garnishment_rate == GARNISHMENT_PERCENTAGE

    def test_custom_garnishment_rate(self, player_repo):
        """Test that custom garnishment rate is applied."""
        service = GarnishmentService(player_repo, garnishment_rate=0.75)
        assert service.garnishment_rate == 0.75

    def test_zero_garnishment_rate(self, player_repo, test_player):
        """Test that zero garnishment rate means no garnishment."""
        service = GarnishmentService(player_repo, garnishment_rate=0.0)
        player_repo.update_balance(test_player, TEST_GUILD_ID, -100)
...
class TestGarnishmentGuildIsolation:
    ...
    def test_garnishment_uses_correct_guild_balance(self, player_repo):
        """Test that garnishment checks balance in the correct guild."""
        service = GarnishmentService(player_repo, garnishment_rate=1.0)
        ...
        player_repo.update_balance(player_id, TEST_GUILD_ID, -50)  # In debt
        player_repo.update_balance(player_id, 67890, 100)  # Positive

        # Add income to each guild
        result1 = service.add_income(player_id, 30, TEST_GUILD_ID)
        result2 = service.add_income(player_id, 30, 67890)
