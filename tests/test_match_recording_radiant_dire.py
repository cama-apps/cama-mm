...
class TestRadiantDireMapping:
    ...
    def test_team_id_mapping_after_shuffle(self, test_db):
        ...
        for pid in team1_ids:
            ...
            assert player.losses == 0, f"Player {pid} (Team 1) should have 0 losses, got {player.losses}"
        ...
        for pid in team2_ids:
            ...
            assert player.losses == 1, f"Player {pid} (Team 2) should have 1 loss, got {player.losses}"
...
class TestRadiantDireBugFix:
    ...
    def test_exact_bug_scenario_dire_wins(self, test_db):
        ...
        radiant_team_ids = [pid for pid, name, _ in player_data if name in [n for n, _ in radiant_players]]
        dire_team_ids = [pid for pid, name, _ in player_data if name in [n for n, _ in dire_players]]
        
        # Verify we have the right players
        assert len(radiant_team_ids) == 5, f"Expected 5 Radiant players, got {len(radiant_team_ids)}"
        ...
        assert reporter_player.wins == 1, f"BUG: BugReporter should have 1 win (Dire won), got {reporter_player.wins}"
        assert reporter_player.losses == 0, f"BUG: BugReporter should have 0 losses (Dire won), got {reporter_player.losses}"
    ...
    def test_exact_bug_scenario_radiant_wins(self, test_db):
        ...
        for pid in radiant_team_ids:
            player = test_db.get_player(pid)
            assert player.wins == 1, f"Radiant player {player.name} should have 1 win, got {player.wins}"
            assert player.losses == 0, f"Radiant player {player.name} should have 0 losses, got {player.losses}"
        
        # Verify Dire players (who lost) have LOSSES
        for pid in dire_team_ids:
            player = test_db.get_player(pid)
            assert player.wins == 0, f"Dire player {player.name} should have 0 wins, got {player.wins}"
            assert player.losses == 1, f"Dire player {player.name} should have 1 loss, got {player.losses}"
