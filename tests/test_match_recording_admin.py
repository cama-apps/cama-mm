...
class TestAdminOverride:
    ...
    def test_has_admin_submission_with_no_submissions(self, match_service, test_db, test_players):
        ...
        assert match_service.has_admin_submission(TEST_GUILD_ID) is False

    def test_has_admin_submission_with_non_admin_submission(
        self, match_service, test_db, test_players
    ):
        """Test has_admin_submission returns False when only non-admin submits."""
        match_service.shuffle_players(test_players, guild_id=TEST_GUILD_ID)
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1001, result="radiant", is_admin=False)
    ...
    def test_has_admin_submission_with_admin_submission(self, match_service, test_db, test_players):
        ...
        match_service.shuffle_players(test_players, guild_id=TEST_GUILD_ID)
        match_service.add_record_submission(TEST_GUILD_ID, user_id=9999, result="radiant", is_admin=True)

        assert match_service.has_admin_submission(TEST_GUILD_ID) is True
    ...
    def test_can_record_match_with_admin_override(self, match_service, test_db, test_players):
        ...
        assert match_service.can_record_match(TEST_GUILD_ID) is True
        assert match_service.get_non_admin_submission_count(TEST_GUILD_ID) == 0
    ...
    def test_can_record_match_without_admin_requires_3_non_admin(
        ...
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1001, result="radiant", is_admin=False)
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1002, result="radiant", is_admin=False)
        ...
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1003, result="radiant", is_admin=False)

        assert match_service.can_record_match(TEST_GUILD_ID) is True
        assert match_service.get_non_admin_submission_count(TEST_GUILD_ID) == 3
    ...
    def test_admin_override_allows_immediate_recording(self, match_service, test_db, test_players):
        ...
        submission = match_service.add_record_submission(
        ...
        record_result = match_service.record_match("radiant", guild_id=TEST_GUILD_ID)
...
class TestFirstToThreeVoting:
    ...
    def test_get_vote_counts_empty(self, match_service, test_db, test_players):
        ...
        counts = match_service.get_vote_counts(TEST_GUILD_ID)
    ...
    def test_get_vote_counts_tracks_votes(self, match_service, test_db, test_players):
        ...
        # Add some votes
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1001, result="radiant", is_admin=False)
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1002, result="dire", is_admin=False)
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1003, result="radiant", is_admin=False)
    ...
    def test_first_to_3_radiant_wins(self, match_service, test_db, test_players):
        ...
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1002, result="dire", is_admin=False)
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1003, result="radiant", is_admin=False)
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1004, result="dire", is_admin=False)

        assert match_service.can_record_match(TEST_GUILD_ID) is False
        assert match_service.get_pending_record_result(TEST_GUILD_ID) is None
        ...
        assert match_service.can_record_match(TEST_GUILD_ID) is True
        assert match_service.get_pending_record_result(TEST_GUILD_ID) == "radiant"
    ...
    def test_first_to_3_dire_wins(self, match_service, test_db, test_players):
        ...
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1001, result="radiant", is_admin=False)
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1002, result="dire", is_admin=False)
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1003, result="dire", is_admin=False)
        ...
        assert match_service.get_pending_record_result(TEST_GUILD_ID) == "dire"
    ...
    def test_user_cannot_change_vote(self, match_service, test_db, test_players):
        ...
        with pytest.raises(ValueError, match="already submitted"):
            match_service.add_record_submission(TEST_GUILD_ID, user_id=1001, result="dire", is_admin=False)
    ...
    def test_first_to_3_records_correct_winner(self, match_service, test_db, test_players):
        ...
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1002, result="radiant", is_admin=False)
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1003, result="dire", is_admin=False)
        match_service.add_record_submission(TEST_GUILD_ID, user_id=1004, result="radiant", is_admin=False)
        submission = match_service.add_record_submission(
            TEST_GUILD_ID, user_id=1005, result="radiant", is_admin=False
        )
...
class TestAbortVoting:
    ...
    def test_non_admin_abort_requires_three_votes(self, match_service, test_db, test_players):
        match_service.shuffle_players(test_players, guild_id=TEST_GUILD_ID)
        assert match_service.can_abort_match(TEST_GUILD_ID) is False

        match_service.add_abort_submission(TEST_GUILD_ID, user_id=1001, is_admin=False)
        match_service.add_abort_submission(TEST_GUILD_ID, user_id=1002, is_admin=False)

        assert match_service.can_abort_match(TEST_GUILD_ID) is False
        submission = match_service.add_abort_submission(TEST_GUILD_ID, user_id=1003, is_admin=False)
        assert submission["is_ready"] is True
        assert match_service.can_abort_match(TEST_GUILD_ID) is True

    def test_admin_abort_overrides_minimum(self, match_service, test_db, test_players):
        match_service.shuffle_players(test_players, guild_id=TEST_GUILD_ID)
        submission = match_service.add_abort_submission(TEST_GUILD_ID, user_id=9999, is_admin=True)

        assert submission["is_ready"] is True
        assert match_service.can_abort_match(TEST_GUILD_ID) is True
        assert submission["non_admin_count"] == match_service.get_abort_submission_count(TEST_GUILD_ID)

    def test_clear_abort_state_after_abort(self, match_service, test_db, test_players):
        match_service.shuffle_players(test_players, guild_id=TEST_GUILD_ID)
        match_service.add_abort_submission(TEST_GUILD_ID, user_id=1001, is_admin=False)
        match_service.add_abort_submission(TEST_GUILD_ID, user_id=1002, is_admin=False)
        match_service.add_abort_submission(TEST_GUILD_ID, user_id=1003, is_admin=False)
        assert match_service.can_abort_match(TEST_GUILD_ID) is True
