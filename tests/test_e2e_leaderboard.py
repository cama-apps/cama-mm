...
class TestLeaderboardEdgeCases:
    ...
    def test_leaderboard_with_ties(self, test_db):
        ...
        for player, pid in players_with_ids:
            ...
            cama_rating = rating_system.rating_to_display(player.glicko_rating) if player.glicko_rating else None
            jopacoin_balance = test_db.get_player_balance(pid)
            players_with_stats.append((player, jopacoin_balance, player.wins, player.losses, win_rate, cama_rating))
        
        players_with_stats.sort(key=lambda x: (x[1], x[2], x[5] if x[5] is not None else 0), reverse=True)
        
        # All should have 100 jopacoin (tied)
        for player, jopacoin, wins, losses, win_rate, rating in players_with_stats:
            assert jopacoin == 100, f"All players should have 100 jopacoin, {player.name} has {jopacoin}"
        ...
        assert ratings == sorted(ratings, reverse=True), "Should be sorted by rating when jopacoin and wins are tied"
    ...
    def test_leaderboard_large_dataset(self, test_db):
        ...
        for i, pid in enumerate(player_ids):
            ...
            cursor.execute("UPDATE players SET jopacoin_balance = ? WHERE discord_id = ?", (jopacoin_balance, pid))
        conn.commit()
        conn.close()
        ...
        assert top_player_jopacoin >= players_with_stats[-1][1], "Top player should have at least as many jopacoin as bottom"
    ...
    def test_leaderboard_sorts_by_jopacoin_then_wins_then_rating(self, test_db):
        ...
        assert players_with_stats[0]["discord_id"] == 400002, "Player2 should be first (100 jopacoin, 5 wins)"
        ...
        assert players_with_stats[1]["discord_id"] == 400001, "Player1 should be second (100 jopacoin, 1 win, 2000 rating)"
        ...
        assert players_with_stats[2]["discord_id"] == 400004, "Player4 should be third (100 jopacoin, 1 win, 1800 rating)"
        ...
        assert players_with_stats[3]["discord_id"] == 400003, "Player3 should be fourth (50 jopacoin, despite 10 wins)"
    ...
    def test_leaderboard_displays_jopacoin(self, test_db):
        ...
        conn = test_db.get_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE players SET jopacoin_balance = 42 WHERE discord_id = ?", (player_id,))
        conn.commit()
        conn.close()
        ...
        if wins + losses > 0:
            stats += f" ({win_rate:.0f}%)"
        rating_display = f" [{cama_rating}]" if cama_rating is not None else ""
        jopacoin_display = f"{jopacoin_balance} {JOPACOIN_EMOTE}"
        
        # Verify jopacoin is in the display
        assert JOPACOIN_EMOTE in jopacoin_display, "Jopacoin display should include the jopacoin emote"
        assert str(jopacoin_balance) in jopacoin_display, f"Jopacoin display should include balance {jopacoin_balance}"
