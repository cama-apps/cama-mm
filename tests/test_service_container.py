...
class TestServiceContainerInitialization:
    ...
    def test_vanity_tax_enforcement_survives_container_restart(self, repo_db_path):
        first = ServiceContainer(repo_db_path)
        first.initialize()
        first_service = first._components["vanity_tax_service"]
        member = type("Member", (), {"id": 7, "nick": "Skater"})()
        first_service.refresh_guild(123, [member])
        first_service.set_manual_taxation(123, 7, enforced=True, actor_id=42)
        ...
        assert second_service.calculate_tax(7, 123, 100) == 10
