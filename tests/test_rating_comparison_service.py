...
class _FakeMatchService:
    ...
    def get_openskill_predictions_for_match(self, team1_ids, team2_ids):
        return {"team1_win_prob": 0.5}


def _service(matches: list[dict] | None = None) -> RatingComparisonService:
    return RatingComparisonService(
        match_repo=_FakeMatchRepo(matches or []),
        player_repo=None,
