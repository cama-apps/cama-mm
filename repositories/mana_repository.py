...
class ManaRepository(BaseRepository, IManaRepository):
    ...
    # ----- mana_shop_items ---------------------------------------------------
    # Routed through the repo so /manashop handlers stop hand-rolling INSERT
    # statements that bypass guild_id normalization.
    ...
    def insert_shop_item(
        ...
        """Persist a mana-shop purchase row."""
        gid = self.normalize_guild_id(guild_id)
        with self.connection() as conn:
            conn.execute(
                """
                INSERT INTO mana_shop_items
                    (discord_id, guild_id, item_type, target_id, purchased_at, expires_at, data)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (discord_id, gid, item_type, target_id, purchased_at, expires_at, data),
            )

    def get_daily_loss_total(
        self, discord_id: int, guild_id: int | None, loss_date: str
    ) -> int:
        """Return the player's total JC lost on ``loss_date`` (0 if no row)."""
        gid = self.normalize_guild_id(guild_id)
        with self.connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
                SELECT total_lost
                FROM mana_daily_losses
                WHERE discord_id = ? AND guild_id = ? AND loss_date = ?
                """,
                (discord_id, gid, loss_date),
            )
            row = cursor.fetchone()
            return int(row["total_lost"]) if row else 0
