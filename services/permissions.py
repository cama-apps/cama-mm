...
from config import ADMIN_USER_IDS, TAX_MAN_USER_IDS
...
def has_allowlisted_tax_man(interaction: discord.Interaction) -> bool:
    """
    Check if the user is explicitly allowlisted as a Tax Man.
    """
    return interaction.user.id in TAX_MAN_USER_IDS
...
def has_tax_man_permission(interaction: discord.Interaction) -> bool:
    """
    Check if user can use Tax Man economy audit/enforcement commands.

    Admins are Tax Men by default. TAX_MAN_USER_IDS/TAX_MEN_USER_IDS can add
    non-admin users without granting full admin permissions.
    """
    return has_admin_permission(interaction) or has_allowlisted_tax_man(interaction)
