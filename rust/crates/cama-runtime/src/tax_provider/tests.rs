...
async fn player_reports_registered_audit_and_exact_missing_target_copy() {
    ...
    assert_eq!(
        ...
        Some("Tax Man Player Audit - Unknown player")
    );
...
async fn resetcooldown_and_bankruptcy_mutate_the_live_database() {
    ...
    assert_eq!(
        ...
        "Reset Tax Man fine cooldown for Unknown player."
    );
    assert_eq!(
        fixture
            .connection()
    ...
    assert!(
        add.followups.lock().expect("followups")[0]
            .content
            .contains("Added 3 bankruptcy modifier game(s) to Unknown player")
    );
    assert_eq!(
        fixture
            .connection()
    ...
    assert!(
        remove.followups.lock().expect("followups")[0]
            .content
            .contains("Removed bankruptcy modifier from Unknown player")
    );
    assert_eq!(
        fixture
            .connection()
