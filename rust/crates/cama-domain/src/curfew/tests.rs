...
mod mode_helper_tests {
    ...
    #[test]
    fn test_retired_modes_are_rejected() {
        assert!(parse_mode("tax").is_err());
        assert!(parse_mode("informational").is_err());
        assert!(parse_mode("info").is_err());
    }
}
...
mod retains_coverage_timezone_tests {
    ...
    fn test_mode_alone_does_not_affect_coverage() {
        let now = ny(2026, 3, 10, 12, 0);
        let default_mode = CurfewWindow {
            mode: CurfewMode::Default,
            ..strict(22, 6)
        };
        assert!(retains_coverage(&default_mode, &strict(22, 6), None, now));
    }
