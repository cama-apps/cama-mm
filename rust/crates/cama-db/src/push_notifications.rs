...
impl PushNotificationRepository {
    ...
    pub fn get_config(
        ...
        Ok(self
            .connection()?
            .query_row(
                "SELECT ntfy_server,ntfy_topic,readycheck_enabled,lobby_enabled
                ...
                |row| {
                    Ok(PushNotificationConfig {
                        ...
                        readycheck_enabled: row.get(2)?,
                        lobby_enabled: row.get(3)?,
                    })
                },
            )
            .optional()?)
    ...
    /// Insert or replace the delivery target, always re-enabling both alert
    /// kinds so a re-run `/notify set` recovers from an earlier `clear`.
    pub fn set_target(
        &self,
        discord_id: i64,
        guild_id: Option<i64>,
        ...
        self.connection()?.execute(
            "INSERT INTO push_notification_targets
                 (discord_id,guild_id,ntfy_server,ntfy_topic,readycheck_enabled,lobby_enabled,updated_at)
             VALUES (?1,?2,?3,?4,1,1,?5)
             ON CONFLICT(discord_id,guild_id) DO UPDATE SET
                 ntfy_server=excluded.ntfy_server,
                 ntfy_topic=excluded.ntfy_topic,
                 readycheck_enabled=1,
                 lobby_enabled=1,
    ...
    pub fn enabled_targets(
        ...
        let placeholders = discord_ids
            ...
            .join(",");
        let sql = format!(
            "SELECT discord_id,ntfy_server,ntfy_topic FROM push_notification_targets
