...
mod tests {
    ...
    #[test]
    #[ignore = "explicit Python-migrated disposable SQLite interoperability smoke"]
    fn python_migrated_disposable_db_pipeline_smoke() {
        use std::process::Command;
        use std::time::{SystemTime, UNIX_EPOCH};

        use cama_db::herogrid_repository::HeroGridRepository;
        ...
        let path = std::env::temp_dir().join(format!(
            "cama-herogrid-python-smoke-{}-{nonce}.db",
            std::process::id()
        ));
        ...
        let repository_root = manifest
            .ancestors()
            .nth(3)
            .expect("cama-app is nested under rust/crates");
        let script = r#"
import os
...
path = os.environ["CAMA_HEROGRID_SMOKE_DB"]
database = Database(path)
players = PlayerRepository(path)
matches = MatchRepository(path)
players.add(discord_id=100, discord_username="Alice", guild_id=12345)
players.add(discord_id=200, discord_username="Bob", guild_id=12345)
match_id = matches.record_match(
    team1_ids=[100], team2_ids=[200], winning_team=1, guild_id=12345
)
matches.update_participant_stats(
...
if database._anchor_connection:
    database._anchor_connection.execute("PRAGMA wal_checkpoint(TRUNCATE)")
    database._anchor_connection.close()
"#;
        let output = Command::new("uv")
            .current_dir(repository_root)
            .args(["run", "--locked", "python", "-c", script])
            .env("CAMA_HEROGRID_SMOKE_DB", &path)
            .output()
            .expect("run Python migration authority");
        assert!(
        ...
        let outcome = HeroGridCommandService::new(HeroGridRepository::new(&path))
            .execute(&HeroGridCommandRequest {
                source: HeroGridSource::Auto,
            ...
            .expect("read Python-migrated database and render");
        let HeroGridCommandOutcome::Attachment(attachment) = outcome else {
            panic!("expected Python→SQLite→Rust→PNG attachment");
        };
        let decoded = decode_png(&attachment.png);
        assert_eq!((decoded.raster.width, decoded.raster.height), (238, 318));
        ...
        for candidate in [
            ...
            if candidate.exists() {
                std::fs::remove_file(candidate).expect("remove disposable smoke database");
            }
        }
    }
}
