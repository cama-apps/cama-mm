...
mod tests {
    ...
    const EXAMPLE_OID_BER: &[u8] = &[0x2A, 0x86, 0x48, 0xCE, 0x3D, 0x02, 0x01];

    #[test]
    fn base128_boundaries_round_trip_without_aliasing() {
        ...
        for arc in [
            ...
            let oid = ObjectIdentifier::from_arcs([1, 2, arc]).unwrap();
            assert_eq!(ObjectIdentifier::from_bytes(oid.as_bytes()).unwrap(), oid);
            assert_eq!(oid.arc(2), Some(arc));
        }
        assert_eq!(
            ObjectIdentifier::from_arcs([1, 2, 128]).unwrap().as_bytes(),
            &[42, 0x81, 0]
        );
    }
