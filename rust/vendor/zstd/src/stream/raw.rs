...
impl<'a> Decoder<'a> {
    ...
    ///
    /// The prepared dictionary must outlive the stream using it.
    ///
    /// ```compile_fail
    /// use zstd::stream::raw::Operation;
    /// let dictionary = zstd::dict::DecoderDictionary::copy(b"dictionary bytes");
    /// let mut stream = zstd::stream::raw::Decoder::with_prepared_dictionary(&dictionary).unwrap();
    /// drop(dictionary);
    /// let _ = stream.run_on_buffers(b"input", &mut [0u8; 128]);
    /// ```
...
impl<'a> Encoder<'a> {
    ...
    /// ```compile_fail
    /// use zstd::stream::raw::Operation;
    /// let dictionary = zstd::dict::EncoderDictionary::copy(b"dictionary bytes", 1);
    /// let mut stream = zstd::stream::raw::Encoder::with_prepared_dictionary(&dictionary).unwrap();
    /// drop(dictionary);
    /// let _ = stream.run_on_buffers(b"input", &mut [0u8; 128]);
    /// ```
    pub fn with_prepared_dictionary<'b>(
