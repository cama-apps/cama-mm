...
#[test]
#[cfg(feature = "std")]
fn cama_nonzero_output_cursor_initializes_unwritten_prefix() {
    let mut bytes = Vec::with_capacity(16);
    bytes.extend_from_slice(b"ok");
    let output = crate::OutBuffer::around_pos(&mut bytes, 8);
    assert_eq!(output.as_slice(), b"ok\0\0\0\0\0\0");
}

#[test]
#[cfg(feature = "std")]
fn cama_empty_decode_preserves_initialized_output_cursor() {
    let mut bytes = Vec::with_capacity(16);
    let mut output = crate::OutBuffer::around_pos(&mut bytes, 8);
    let mut input = crate::InBuffer::around(&[]);
    let mut context = crate::DCtx::create();
    context.decompress_stream(&mut output, &mut input).unwrap();
    assert_eq!(output.as_slice(), &[0; 8]);
}
#[test]
#[should_panic(expected = "Input position outside buffer bounds")]
fn cama_public_input_cursor_is_validated_before_ffi() {
    let mut input = crate::InBuffer {
        src: &[],
        pos: usize::MAX,
    };
    let mut data = [0u8; 8];
    let mut output = crate::OutBuffer::around(&mut data[..]);
    let mut context = crate::DCtx::create();
    let _ = context.decompress_stream(&mut output, &mut input);
}
