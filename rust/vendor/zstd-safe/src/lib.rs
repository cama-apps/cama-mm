...
impl<'a> CCtx<'a> {
    ...
    #[cfg_attr(feature = "doc-cfg", doc(cfg(feature = "experimental")))]
    #[deprecated]
    pub fn init_using_cdict<'b>(&mut self, cdict: &'a CDict<'b>) -> SafeResult
    where
    ...
    ///
    /// ```compile_fail
    /// let dictionary = zstd_safe::CDict::create(b"dictionary bytes", 1);
    /// let mut context = zstd_safe::CCtx::create();
    /// context.ref_cdict(&dictionary).unwrap();
    /// drop(dictionary);
    /// let mut output = [0u8; 128];
    /// context.compress2(&mut output[..], b"dictionary bytes").unwrap();
    /// ```
...
impl<'a> DCtx<'a> {
    ...
    #[cfg_attr(feature = "doc-cfg", doc(cfg(feature = "experimental")))]
    #[deprecated]
    pub fn init_using_ddict<'b>(&mut self, ddict: &'a DDict<'b>) -> SafeResult
    where
        'b: 'a,
    {
        let code = unsafe {
    ...
    ///
    /// ```compile_fail
    /// let dictionary = zstd_safe::DDict::create(b"dictionary bytes");
    /// let mut context = zstd_safe::DCtx::create();
    /// context.ref_ddict(&dictionary).unwrap();
    /// drop(dictionary);
    /// let mut output = [0u8; 128];
    /// let _ = context.decompress(&mut output[..], b"compressed bytes");
    /// ```
...
impl<'a, C: WriteBuf + ?Sized> OutBuffer<'a, C> {
    ...
    /// Any previously uninitialized prefix up to `pos` is initialized to zero.
    ///
    /// # Panics
    ///
    /// If `pos > dst.capacity()`.
    pub fn around_pos(dst: &'a mut C, pos: usize) -> Self {
        ...
        // A nonzero starting cursor must not expose uninitialized Vec capacity
        // when a native call produces no output but updates its position.
        let initialized = dst.as_slice().len();
        if pos > initialized {
            // Safety: initialized <= pos <= capacity; this exclusive buffer
            // owns writable storage for every byte in that range.
            unsafe {
                dst.as_mut_ptr()
                    .add(initialized)
                    .write_bytes(0, pos - initialized);
                dst.filled_until(pos);
            }
        }
        OutBuffer { dst, pos }
    }
...
impl<'a> InBuffer<'a> {
    ...
    fn wrap<'b>(&'b mut self) -> InBufferWrapper<'b, 'a> {
        // These fields are public, so set_pos() is not the only way to change
        // the cursor. Validate before C performs pointer arithmetic.
        assert!(
            self.pos <= self.src.len(),
            "Input position outside buffer bounds"
        );
        InBufferWrapper {
            buf: zstd_sys::ZSTD_inBuffer {
