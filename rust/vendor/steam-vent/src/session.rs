...
async fn send_logon(
    ...
    Ok(Session {
        ...
        job_id: JobIdCounter::default(),
        heartbeat_interval: validated_heartbeat_interval(response.heartbeat_seconds())?,
        app_id: None,
        access_token,
    })
}
...
fn validated_heartbeat_interval(seconds: i32) -> Result<Duration, NetworkError> {
...
mod heartbeat_tests {
    ...
    #[test]
    fn session_debug_never_exposes_access_token() {
        let session = Session {
            access_token: Some("session-token-sentinel".into()),
            ..Session::default()
        };
        let debug = format!("{session:?}");
        assert!(debug.contains("Session"));
        assert!(!debug.contains("session-token-sentinel"));
    }

    #[test]
    fn peer_cannot_trigger_zero_or_overflowing_heartbeat_sleep() {
        for seconds in [i32::MIN, -1, 0, 301, i32::MAX] {
            assert!(validated_heartbeat_interval(seconds).is_err());
        }
        assert_eq!(
            validated_heartbeat_interval(15).unwrap(),
