...
impl Header {
    pub fn validate(&self) -> Result<()> {
        if self.magic != MAGIC || self.length as usize > crate::net::MAX_MESSAGE_SIZE {
...
pub async fn encode_message<T: NetMessage, S: Sink<BytesMut, Error = NetworkError> + Unpin>(
    ...
    trace!("encoded message({} bytes)", buff.len());
    dst.send(buff).await?;
        ...
        flatten_multi(
            raw_reader
                .and_then(move |encrypted| {
                    let decrypted = if encrypted.len() < 32 || encrypted.len() % 16 != 0 {
                        Err(NetworkError::InvalidHeader)
                    } else {
