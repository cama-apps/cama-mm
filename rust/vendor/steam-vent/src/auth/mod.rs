...
impl std::fmt::Debug for SteamGuardToken {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("SteamGuardToken([REDACTED])")
    }
}
...
impl PendingAuth {
    pub(crate) async fn wait_for_tokens(
        ...
        let interval = validated_poll_interval(self.interval)?;
        loop {
            let mut response =
                poll_until_info(connection, self.client_id, &self.request_id, interval).await?;
...
fn validated_poll_interval(interval: f32) -> Result<Duration, NetworkError> {
    // Reject peer-provided NaN/infinity/negative/zero values before converting;
    // do not allow a zero-delay busy loop or indefinitely suspended login.
    if !interval.is_finite() || !(0.1..=60.0).contains(&interval) {
        return Err(NetworkError::InvalidHeader);
    }
    Duration::try_from_secs_f32(interval).map_err(|_| NetworkError::InvalidHeader)
}
...
mod interval_tests {
    ...
    fn credential_debug_output_is_redacted() {
        assert_eq!(
            format!("{:?}", Token("fixture-sensitive-token".into())),
            "Token([REDACTED])"
        );
        assert_eq!(
            format!("{:?}", SteamGuardToken("fixture-code".into())),
            "SteamGuardToken([REDACTED])"
        );
    }

    #[test]
    fn rejects_invalid_peer_poll_intervals_without_panicking() {
        for interval in [
            f32::NAN,
            f32::INFINITY,
            f32::NEG_INFINITY,
            ...
            assert!(validated_poll_interval(interval).is_err());
        }
        assert_eq!(
            validated_poll_interval(5.0).unwrap(),
