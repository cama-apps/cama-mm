...
// Bound both individual CM frames and protobuf metadata before allocating.
// Dota lobby and postgame messages fit well below these limits.
        ...
        if is_protobuf {
            ...
            let header = if header_length > 0 {
                ...
                if header_length as usize > MAX_HEADER_SIZE
                    || u64::from(header_length) > end.saturating_sub(position)
                {
...
impl RawNetMessage {
    pub fn read<Body: Into<Bytes>>(body: Body) -> Result<Self> {
        ...
        let mut value = BytesMut::from(body);
        ...
        if body_start < header_start || body_start > value.len() {
            return Err(NetworkError::InvalidHeader);
        }
    ...
    pub fn from_message_with_kind<T: EncodableMessage, K: MsgKindEnum>(
        ...
        trace!("encoded body({} bytes)", buff.len());
...
mod bounds_tests {
    ...
    fn frame(length: u32, body: &[u8]) -> Vec<u8> {
        let mut frame = (PROTO_MASK | 1).to_le_bytes().to_vec();
    ...
    #[test]
    fn rejects_declared_header_larger_than_available_frame() {
        assert!(RawNetMessage::read(frame(u32::MAX, &[])).is_err());
        assert!(RawNetMessage::read(frame(2, &[0])).is_err());
    }

    #[test]
    fn rejects_header_over_limit_even_if_frame_contains_it() {
        assert!(
            RawNetMessage::read(frame(
                MAX_HEADER_SIZE as u32 + 1,
                &vec![0; MAX_HEADER_SIZE + 1],
            ))
            .is_err()
        );
    }
    ...
    #[test]
    fn empty_protobuf_header_preserves_body() {
        let message = RawNetMessage::read(frame(0, &[1, 2, 3])).unwrap();
        assert_eq!(&message.data[..], &[1, 2, 3]);
    }

    #[test]
    fn every_truncation_of_legacy_header_returns_error() {
        let mut frame = Vec::new();
        NetMessageHeader::default()
            .write(&mut frame, EMsg::k_EMsgClientHeartBeat, false)
            .unwrap();
        assert!(RawNetMessage::read(frame.clone()).is_ok());
        for len in 0..frame.len() {
            assert!(RawNetMessage::read(frame[..len].to_vec()).is_err());
        }
    }
}
