...
impl SteamID {
    pub fn new(
        ...
        Self(
            (account_id as u64)
                | ((u64::from(instance.0) & 0xFFFFF) << 32)
    ...
    pub fn set_instance(&mut self, instance: Instance) {
        ...
        self.0 |= (u64::from(instance.0) & 0xFFFFF) << 32;
    }
    ...
    pub fn from_steam2(steam2: &str) -> Result<Self, SteamIDParseError> {
        ...
        let account_id = account_id
            .checked_mul(2)
            .and_then(|id| id.checked_add(auth_server))
            .ok_or(SteamIDParseError)?;
    ...
    pub fn steam3(&self) -> String {
        ...
        match account_type {
            ...
            AccountType::Individual => {
                render_instance = instance_type != Some(InstanceType::Desktop)
            }
            _ => (),
        };
...
impl Debug for Instance {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        if let Some(instance_type) = self.instance_type()
            && let Some(flags) = self.flags()
        {
            write!(f, "{{Type: {:?}, Flags: {:?}}}", instance_type, flags)
        } else {
            write!(f, "{}", self.0)
        }
    }
...
#[cfg(test)]
mod identity_bounds_tests {
    use super::*;

    #[test]
    fn steam2_overflow_cannot_alias_another_account() {
        assert!(SteamID::from_steam2("STEAM_1:0:2147483648").is_err());
        assert!(SteamID::from_steam2("STEAM_1:1:4294967295").is_err());
        assert_eq!(
            SteamID::from_steam2("STEAM_1:1:2147483647")
                .unwrap()
                .account_id(),
            u32::MAX
        );
    ...
    #[test]
    fn oversized_instance_cannot_corrupt_account_type_or_universe() {
        ...
        assert_eq!(id.instance().0, 0xFFFFF);
        id.set_instance(Instance(u32::MAX));
        assert_eq!(id.account_type(), AccountType::Individual);
        assert_eq!(id.universe(), Universe::Public);
        assert_eq!(id.account_id(), 123);
        assert!(SteamID::from_steam64(id.steam64()).is_ok());
    }
}
