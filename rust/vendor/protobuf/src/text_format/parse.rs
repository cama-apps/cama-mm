...
impl<'a> Parser<'a> {
    ...
    fn read_message(&mut self, descriptor: &MessageDescriptor) -> ParseResult<Box<dyn MessageDyn>> {
        // Keep text parsing bounded like the binary stream's default limit.
        if self.recursion_level >= 100 {
            return Err(ParseErrorWithoutLoc::OverRecursionLimit);
        }
        self.recursion_level += 1;
        let result = self.read_message_no_depth_check(descriptor);
        self.recursion_level -= 1;
        result
    }

    fn read_message_no_depth_check(
        &mut self,
...
#[cfg(test)]
mod cama_recursion_tests {
    use super::*;
    use crate::descriptor::DescriptorProto;

    #[test]
    fn deeply_nested_text_message_is_rejected() {
        let input = format!("{}{}", "nested_type {".repeat(1_000), "}".repeat(1_000));
        let error = parse_from_str::<DescriptorProto>(&input).unwrap_err();
        assert!(error.to_string().contains("Over recursion limit"));
    }

    #[test]
    fn shallow_nested_text_message_is_accepted() {
        let message = parse_from_str::<DescriptorProto>(
            "nested_type { name: \"outer\" nested_type { name: \"inner\" } }",
        )
        .unwrap();
        assert_eq!(message.nested_type[0].nested_type[0].name(), "inner");
    }

    #[test]
    fn text_message_error_restores_recursion_budget() {
        let mut parser = Parser {
            tokenizer: Tokenizer::new("nested_type { unknown: 1 }", ParserLanguage::TextFormat),
            recursion_level: 0,
        };
        assert!(parser.merge(&mut DescriptorProto::new()).is_err());
        assert_eq!(parser.recursion_level, 0);
    }
}
