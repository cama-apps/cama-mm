...
impl<'a> CodedInputStream<'a> {
    ...
    pub fn merge_message_dyn(&mut self, message: &mut dyn MessageDyn) -> crate::Result<()> {
        self.incr_recursion()?;
        let ret = self.merge_message_dyn_no_depth_check(message);
        self.decr_recursion();
        ret
    }

    fn merge_message_dyn_no_depth_check(
        &mut self,
...
mod test {
    ...
    #[test]
    fn cama_generated_message_rejects_deep_unknown_group() {
        use crate::well_known_types::empty::Empty;
        use crate::Message;

        // A known message's unknown-field branch reaches the same group parser
        // used by the generated Steam and Dota protocol types.
        let mut bytes = vec![0x0b; 1_000];
        bytes.extend(vec![0x0c; 1_000]);
        let error = Empty::parse_from_bytes(&bytes).unwrap_err();
        assert!(error.to_string().contains("Over recursion limit"));
    }

    #[test]
    fn cama_generated_message_accepts_shallow_unknown_group() {
        use crate::well_known_types::empty::Empty;
        use crate::Message;

        let bytes = [0x0b, 0x0b, 0x0c, 0x0c];
        assert!(Empty::parse_from_bytes(&bytes).is_ok());
    }

    #[test]
    fn cama_unknown_group_error_restores_recursion_budget() {
        let bytes = [0x0b, 0x0b, 0x0c, 0x0c];
        let mut input = CodedInputStream::from_bytes(&bytes);
        input.set_recursion_limit(1);
        assert!(input.skip_group().is_err());
        assert_eq!(input.recursion_level, 0);
    }

    #[test]
    fn cama_dynamic_message_respects_recursion_limit() {
        ...
        field.set_name("child".into());
        field.set_number(1);
        field.set_label(Label::LABEL_OPTIONAL);
        field.set_type(Type::TYPE_MESSAGE);
        field.set_type_name(".Node".into());
        let mut node = DescriptorProto::new();
        node.set_name("Node".into());
        node.field.push(field);
        let mut file = FileDescriptorProto::new();
        file.set_name("recursion.proto".into());
        file.message_type.push(node);
        let descriptor = FileDescriptor::new_dynamic(file, &[]).unwrap();
        let node = descriptor.message_by_package_relative_name("Node").unwrap();

        let mut bytes = Vec::new();
        for _ in 0..1_000 {
            let mut outer = Vec::new();
            let mut output = CodedOutputStream::vec(&mut outer);
            output.write_raw_byte(0x0a).unwrap();
            output.write_raw_varint32(bytes.len() as u32).unwrap();
            output.write_raw_bytes(&bytes).unwrap();
            output.flush().unwrap();
            drop(output);
            bytes = outer;
        }
        let mut input = CodedInputStream::from_bytes(&bytes);
        let mut message = node.new_instance();
        let error = message.merge_from_dyn(&mut input).unwrap_err();
        assert!(error.to_string().contains("Over recursion limit"));
        assert_eq!(input.recursion_level, 0);

        assert!(node.parse_from_bytes(&[0x0a, 0]).is_ok());
    }
}
