    ...
    if s.blockRandomised {
        ...
        'return_notr: loop {
            ...
            if c_state_out_len > 0 {
                ...
                // The public C ABI permits a NULL output pointer when its
                // capacity is zero. Rust pointer intrinsics still require a
                // non-null pointer for a zero-length operation.
                if bound != 0 {
                    unsafe {
...
#[cfg(test)]
mod cama_null_output_tests {
    use super::*;

    #[test]
    fn pending_run_accepts_null_zero_capacity_output() {
        let input = [65u8; 1000];
        let mut compressed = [0u8; 2048];
        let mut compressed_len = compressed.len() as u32;
        // All raw buffers satisfy the public C ABI contracts, including the
        // specifically documented NULL/zero-capacity output configuration.
        unsafe {
            assert_eq!(
            ...
            assert_eq!(BZ2_bzDecompressInit(&mut stream, 0, 0), 0);
            stream.next_in = compressed.as_mut_ptr().cast();
            stream.avail_in = compressed_len;
            let mut byte = 0u8;
            stream.next_out = (&mut byte as *mut u8).cast();
            stream.avail_out = 1;
            assert_eq!(BZ2_bzDecompress(&mut stream), 0);
            assert_eq!(byte, 65);
            assert!((*(stream.state as *const DState)).state_out_len > 0);
            stream.next_out = core::ptr::null_mut();
            stream.avail_out = 0;
            assert_eq!(BZ2_bzDecompress(&mut stream), 0);
            assert_eq!(BZ2_bzDecompressEnd(&mut stream), 0);
        }
    }
}
