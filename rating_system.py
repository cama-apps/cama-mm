...
class CamaRatingSystem:
    ...
    def create_player_from_mmr(self, mmr: Optional[int]) -> Player:
        """
    ...
    def create_player_from_rating(self, 
                                   rating: float, 
                                   rd: float, 
                                   volatility: float) -> Player:
        """
    ...
    def update_ratings_after_match(self,
                                   team1_players: List[Tuple[Player, int]],  # (player, discord_id)
                                   team2_players: List[Tuple[Player, int]],
                                   winning_team: int) -> Tuple[List[Tuple[float, float, float, int]], 
        ...
        def _dampen_delta(delta: float, team_rating: float, opponent_rating: float, result: float) -> float:
            # If a heavy favorite loses, soften the drop to avoid over-penalizing.
            if result == 0.0 and (team_rating - opponent_rating) > 200:
                return delta * 0.05
            return delta

        team1_delta = _dampen_delta(team1_synthetic.rating - team1_rating, team1_rating, team2_rating, team1_result)
        team2_delta = _dampen_delta(team2_synthetic.rating - team2_rating, team2_rating, team1_rating, team2_result)
